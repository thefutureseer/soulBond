...
async function main() {
  ...
  const users = await prisma.user.createMany({
    ...
    skipDuplicates: true, // Skip creating users if they already exist
  });

  console.log(`${users.count} users created or already exist!`);
  ...
  const promises = await prisma.soulpromise.createMany({
    data: [
      {
    ...
    skipDuplicates: true, // Skip creating promises if they already exist
  });

  console.log("Seeding completed with soulful promises!");
  console.log(`${promises.count} promises created or already exist!`);
  
  // Retrieve the promises after seeding
  const allPromises = await prisma.soulpromise.findMany();

  // Create a map of titles to IDs for easy lookup
  const promiseMap = new Map(allPromises.map(promise => [promise.title, promise.id]));

  // Seed edits for the promises
  await prisma.soulpromise.createMany({
    data: [
      {
        title: "Daily Meditation - Edit 1",
        description: "I promise to meditate for at least 15 minutes every day.",
        editedById: existingUserId1,
        version: 2,
        status: "PENDING",
        parentId: allPromises.find(p => p.title === "Daily Meditation")?.id, // Link to the parent promise
      },
      {
        title: "Spread Kindness - Edit 1",
        description: "I promise to perform two acts of kindness every day.",
        editedById: existingUserId2,
        version: 2,
        status: "COMPLETED",
        parentId: allPromises.find(p => p.title === "Spread Kindness")?.id, // Link to the parent promise
      },
      // Add more edits as needed
    ],
  });

  console.log("Seeding completed with soulful promises and edits!");
